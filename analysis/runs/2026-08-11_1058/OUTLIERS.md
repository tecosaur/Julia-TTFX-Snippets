...
**Cause found: invalidation of precompiled code, not slow compilation.** On nightly the call is 99.5% `recompile_time` — GMT's pkgimage `plot` path is discarded and re-inferred on first use — against exactly zero recompilation on 1.12 and 1.13. Counting invalidations during `using GMT`, the classic method-invalidation log *shrinks* while pkgimage **edge-validation** events go 74 (1.13) → 5959 (nightly), an ~80x jump. `ComponentArrays` is flat across the same boundary (282 → 261), so this is not universal.

Note this is the opposite mechanism to the ComponentArrays finding below, where edge validation contributed nothing and the classic path exploded — two independent invalidation regressions in the same nightly. JuliaLang/julia#61667 already tracks GMT invalidations for the 1.12 → 1.13 step (root causes: a binding invalidation of a const-propagated `Base.isvisible`, and static backedges from abstract iteration introduced by the `iterate_starting_state` split). Neither explains the 1.14 jump, and JuliaLang/julia#62658 leaves the edge-validation count byte-identical at 5959, so that escalation looks separate. Details: [`../../ISSUE-gmt-edge-invalidation.md`](../../ISSUE-gmt-edge-invalidation.md).
...
**JuliaLang/julia#62658 fixes most of it.** That PR applies `concrete_only` to `!=`, `>`, `>=`, `|` and `&`; `!=` and `>=` are among the triggers here. On that build the leaf count drops 208 → 68, recompilation 23.8s → 1.21s, and Static's precompile 32.5s → 6.7s — no package change needed. A residual gap remains against 1.11's 1.2s, so dropping the macro is still worthwhile. Details, with reproducers: [`../../ISSUE-recompile-invalidations.md`](../../ISSUE-recompile-invalidations.md).
